...
describe.only('PyPI', function() {
  ...
  it.skip('should be able to fetch raw XML of recent packages feed', async function() {
    this.timeout(10000);
    rawXML = await PyPI.fetchXMLFromPyPI();
    expect(rawXML).to.be.a('string');
  });

  it.skip('should be able to parse package names out of the XML', async function() {
  ...
  it.skip('should be able to fetch the Github for a package', async function() {
    this.timeout(10000);
    var packageRepo = await PyPI.packageNameToRepo('cognite-sdk');
    expect(packageRepo).to.equal('cognitedata/cognite-sdk-python');
  });

  it('should upsert recent packages from the entry point', async function() {
    this.timeout(10000);
